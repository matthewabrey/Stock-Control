...
import { ArrowLeft, Save, Trash2, DoorOpen, Refrigerator } from "lucide-react";
...
const StoreDesigner = () => {
  ...
  useEffect(() => {
    drawGrid();
  }, [zones, doors, fridges, isSelecting, selectionStart, selectionEnd, selectedZoneIndex, isDraggingCopy, draggedZoneCopy]);
  ...
  const handleMouseUp = () => {
    if (isSelecting && selectionStart && selectionEnd && mode === "zone") {
      const x = Math.min(selectionStart.x, selectionEnd.x);
      const y = Math.min(selectionStart.y, selectionEnd.y);
